...
interface RiskOverviewDetailProps {
  ticker: string;
  cryptoName: string;
...
export function RiskOverviewDetail({
  ticker,
  cryptoName,
  decisionData,
  liveData,
    ...
    <Card variant="hover" className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">Risk telemetry & potential risk factors for {cryptoName}</p>
        </div>

        <span className={`px-3.5 py-1.5 rounded-xl text-xs font-mono font-extrabold border self-start sm:self-auto ${riskStyle.bg}`}>
