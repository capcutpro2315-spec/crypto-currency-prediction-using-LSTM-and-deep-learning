...
export function AISignalDetail({ ticker, cryptoName, decisionData, loading }: AISignalDetailProps) {
    ...
    <Card variant="gradient" className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Algorithmic signal evaluation and underlying rationale for {cryptoName}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-6 bg-slate-950/60 rounded-2xl border border-slate-800 space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Detailed reasoning is currently unavailable for {cryptoName}.
          </p>
        </div>
      ) : (
